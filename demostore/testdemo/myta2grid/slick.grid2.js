...
//                if (opt.showHeaderRow) {
    ...
    function setupColumnSort() {
      $headers.click(function(e) {
        if ($(e.target).hasClass("slick-resizable-handle") || $(e.target).hasClass("slick-hideshow-handle")||$(e.target).hasClass("slick-filter")) {
          return;
        }
...
// var test = new Date();
    ...
    function resizeCanvas(isNotDefault) {
      ...
      // jiangzh,如果存在列使用totals(sum、avg、max...)属性,则重新计算高度    add by zzb 如果没有数据不减高度
      if(options.hasTotals && self.getDataView().getItems().length!=0){
        viewportH = viewportH - options.topPanelHeight;
      }
...
// 					}
      ...
      //访问Action并提交参数
      location.href=url+"?"+queryStr+"&_grid__export_excelName="+options.exportFileName;
    }
    /**
